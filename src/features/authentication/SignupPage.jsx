...
const SignupPage = ({ setAuthView, onSignup }) => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold text-[var(--color-text-primary)] mb-2">Create an Account</h2>
            <p className="text-[var(--color-text-secondary)] mb-8">Join us to start managing your operations.</p>
            <form onSubmit={handleSignup} className="space-y-6">
                <div className="relative">
                    ...
                    <input value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full pl-10 pr-3 py-3 text-sm bg-[var(--color-input-bg)] text-[var(--color-text-primary)] border border-[var(--color-border)] rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent placeholder-slate-400" type="text" placeholder="Full Name"/>
                </div>
                <div className="relative">
                ...
                <button type="submit" className="w-full inline-flex items-center justify-center rounded-md font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-[var(--color-button-primary-bg)] text-[var(--color-button-primary-text)] hover:opacity-90 px-4 py-3 text-base">Sign Up</button>
            </form>
            <div className="mt-8 text-center">
                <p className="text-sm text-[var(--color-text-secondary)]">Already have an account?
                    <button onClick={() => setAuthView('login')} className="font-semibold text-[var(--color-link)] hover:underline focus:outline-none ml-1">Log in</button>
                </p>
            </div>
        </div>
    );
