            ...
            {paginatedData.length > 0 ? (
                        ...
                        <tbody className="divide-y divide-[var(--color-border)]">
                            {paginatedData.map(item => (
                                <tr key={item.id} className="hover:bg-[var(--color-table-row-hover-bg)] transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-[var(--color-surface-contrast)] text-[var(--color-text-primary)]">{item.site}</span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[var(--color-text-secondary)]">{new Date(item.expiryDate).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' })}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center no-print">
                                        <button onClick={() => handleRenewClick(item.id)} className="bg-[var(--color-button-warning-bg)] text-[var(--color-button-warning-text)] font-bold py-2 px-4 rounded-lg hover:bg-[var(--color-button-warning-hover-bg)] transition-colors text-sm shadow-sm hover:shadow-md transform hover:scale-105">
                                            Renew
                                        </button>
                                    </td>
                                </tr>
                ...
                <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
                 ...
                 <div className="text-center py-10 bg-[var(--color-surface)] rounded-lg shadow-[var(--shadow-card)]">
                    <p className="text-[var(--color-text-secondary)]">No expiring certificates found.</p>
                </div>
            )}
        </div>
    );
