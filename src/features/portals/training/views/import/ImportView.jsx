...
const UploadIcon = (props) => (
    <svg className="w-8 h-8 mb-4 text-[var(--color-text-secondary)]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16" {...props}><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/></svg>
);

// icon: CheckCircleIcon
const CheckCircleIcon = (props) => (
    <svg className="h-6 w-6 text-[var(--color-badge-success-text)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
);
...
const Button_Primary = (props) => (
    <Button
        {...props}
        className="bg-[var(--color-button-info-bg)] text-[var(--color-button-info-text)] shadow-md hover:bg-[var(--color-button-info-hover-bg)] flex items-center"
    />
);
...
const Button_Subtle = (props) => (
    <Button
        {...props}
        className="text-[var(--color-text-primary)] bg-[var(--color-surface-alt)] border border-[var(--color-border)] hover:bg-[var(--color-surface-hover)]"
    />
);

// primitive: Button_Success
const Button_Success = (props) => (
    <Button
        {...props}
        className="bg-[var(--color-success)] text-white font-semibold flex items-center hover:opacity-90"
    />
);

// block: StepperBlock
const StepperBlock = ({ step }) => (
...
const FileUploadBlock = ({ isLoading, handleFileSelect, handleDownloadTemplate, handleBackup }) => (
    <div>
        ...
        <div className="mt-8 p-4 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)]">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                <Button_Subtle onClick={() => handleDownloadTemplate('csv')} className="w-full sm:w-auto">Download CSV Template</Button_Subtle>
                <Button_Subtle onClick={() => handleDownloadTemplate('xlsx')} className="w-full sm:w-auto">Download Excel Template</Button_Subtle>
                <Button_Success onClick={handleBackup} className="w-full sm:w-auto">Share/Backup DB</Button_Success>
            </div>
        </div>
    </div>
);

// block: ColumnMappingBlock
const ColumnMappingBlock = ({ file, appDataFields, mappings, setMappings, fileHeaders, resetState, handleValidate, isLoading }) => (
    <div className="bg-[var(--color-surface)] p-6 rounded-xl shadow-md">
        ...
        <div className="flex justify-end gap-4 mt-8">
            <Button_Subtle onClick={resetState}>Cancel</Button_Subtle>
            <Button_Primary onClick={handleValidate} disabled={isLoading}>
...
const PreviewTableBlock = ({ fileData, transformedData, appDataFields, setStep, handleFinalImport, isLoading }) => (
    <div className="bg-[var(--color-surface)] p-6 rounded-xl shadow-md">
        ...
        <div className="flex justify-end gap-4 mt-8">
            <Button_Subtle onClick={() => setStep(2)} disabled={isLoading}>Back to Mapping</Button_Subtle>
            <Button_Success onClick={() => handleFinalImport()} disabled={isLoading}>
                {isLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>}
...
// block: ImportSuccessBlock
const ImportSuccessBlock = ({ importResult, resetState }) => (
    <div className="bg-[var(--color-surface)] p-8 rounded-xl shadow-md text-center">
        ...
        <Button_Primary onClick={resetState} className="mt-6">Import Another File</Button_Primary>
    </div>
);
...
const ImportPage = ({ logActivity, sites, provinces, certificateTypes, onImportSuccess, settingsHandlers }) => {
    ...
    const handleFileSelect = (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                setError(`Error parsing file: ${err.message}`); resetState();
    ...
    const renderValidationStep = () => (
        <div className="bg-[var(--color-surface)] p-6 rounded-xl shadow-md">
            ...
            <div className="flex justify-end gap-4 mt-8">
                <Button_Subtle onClick={() => setStep(2)} disabled={isLoading}>Back to Mapping</Button_Subtle>
                <Button_Primary onClick={handleCreateNewItemsAndContinue} disabled={isLoading}>
                    {isLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-[var(--color-surface-alt)] p-4 rounded-lg shadow-xl w-11/12 max-w-sm">
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            <Button_Subtle type="button" onClick={() => setShowQuickAddModal(false)} >Cancel</Button_Subtle>
                            <Button_Primary type="button" onClick={handleQuickAddProvince} >Add</Button_Primary>
                        </div>
                    </div>
                </div>
            )}
    ...
    const renderStep = () => {
        switch (step) {
            case 1: return <FileUploadBlock isLoading={isLoading} handleFileSelect={handleFileSelect} handleDownloadTemplate={handleDownloadTemplate} handleBackup={handleBackup} />;
            case 2: return <ColumnMappingBlock file={file} appDataFields={appDataFields} mappings={mappings} setMappings={setMappings} fileHeaders={fileHeaders} resetState={resetState} handleValidate={handleValidate} isLoading={isLoading} />;
            case 2.5: return renderValidationStep();
            case 3: return <PreviewTableBlock fileData={fileData} transformedData={transformedData} appDataFields={appDataFields} setStep={setStep} handleFinalImport={handleFinalImport} isLoading={isLoading} />;
            case 4: return <ImportSuccessBlock importResult={importResult} resetState={resetState} />;
            default: return null;
        }
    }
